    ...
    for j in neighbors:
        ...
        if __name__=='__main__':
            with Pool(12) as p:
                ...
                mi_W = p.map(partial(est.WMI, measure = 'MI', k = j, 
        ...
        MI.to_hdf('MI_bench.h5', key = 'dim_' + str(d) + '_k_' + str(j), mode = 'a')
        
        SD = pd.DataFrame(sd,index=cols, columns = lines).transpose()
        SD.to_hdf('MI_bench_sd.h5', key = 'dim_' + str(d) + '_k_' + str(j), mode = 'a')
